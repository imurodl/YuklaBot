...
class VidZillaBot:
    ...
    def __init__(self):
        ...
        self.use_webhook = bool(WEBHOOK_URL and WEBHOOK_PATH)
    ...
    async def _setup_polling(self) -> None:
        ...
        logger.info("Bot configured for POLLING mode (local development)")
        logger.info("Bot will check for updates every few seconds")
    ...
    async def create_app(self) -> Optional[web.Application]:
        await self._create_bot_and_dispatcher()
        await self._register_handlers()

        if self.use_webhook:
            await self._create_web_app()
            logger.info("Application created successfully (WEBHOOK mode)")
        else:
            await self._setup_polling()
            logger.info("Application created successfully (POLLING mode)")
